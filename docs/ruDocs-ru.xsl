...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<!--
		LSMD v0.3p (XML) -> ruDocs (HTML) Russian
		Copyright(c) Seg@, April 2006
	...
	<xsl:key name="action-search" match="команда" use="@id"/>
	<xsl:key name="bool-search" match="да-нет" use="@id"/>
	<xsl:key name="color-search" match="цвет" use="@id"/>
	<xsl:key name="coordinate-search" match="координата" use="@id"/>
	<xsl:key name="file-search" match="файл" use="@id"/>
	<xsl:key name="flags-search" match="флаги" use="@id"/>
	<xsl:key name="folder-search" match="папка" use="@id"/>
	<xsl:key name="font-search" match="шрифт" use="@id"/>
	<xsl:key name="image-search" match="изображение" use="@id"/>
	<xsl:key name="integer-search" match="число" use="@id"/>
	<xsl:key name="option-search" match="опция" use="@id"/>
	<xsl:key name="key-search" match="ключ" use="@id"/>
	<xsl:key name="sound-search" match="звук" use="@id"/>
	<xsl:key name="string-search" match="строка" use="@id"/>
	...
	<xsl:template name="default-value">
		...
		<xsl:for-each select="/lsmd/module/data">
			<xsl:for-each select="key(concat($datatype,'-search'), $dataid)">
				<br /><u>По-умолчанию</u> : <xsl:value-of select="@default"/>
			</xsl:for-each>
		</xsl:for-each>
	</xsl:template>
	...
	<xsl:template match="module" mode="doc">
		...
		<xsl:if test="not(//section/@id='toc')">
			<xsl:call-template name="section-header">
				...
				<xsl:with-param name="title">Содержание:</xsl:with-param>
			</xsl:call-template>
		...
		<xsl:if test="history and not(//section/@id='changes')">
			<xsl:call-template name="section-header">
				...
				<xsl:with-param name="title">История</xsl:with-param>
			</xsl:call-template>
		...
		<xsl:if test="author">
			<xsl:call-template name="section-header">
				...
				<xsl:with-param name="title">Разработчики</xsl:with-param>
			</xsl:call-template>
	...
	<xsl:template match="documentation" mode="toc">
		...
		<xsl:if test="../history and not(//section/@id='changes')">
			<li>
				<xsl:element name="a">
					<xsl:attribute name="href">#changes</xsl:attribute>История версий</xsl:element>
			</li>
		</xsl:if>
		<li>
			<xsl:element name="a"><xsl:attribute name="href">#dev</xsl:attribute>Разработчики</xsl:element>
		</li>
		</div>
	</xsl:template>
...
<!-- not enough for non-ascii case conversion, but I don't care. -->
	...
	<xsl:template match="fixed" mode="changes">
		<xsl:text>&#13;</xsl:text>
		<li><b>ИСПРАВЛЕНО</b>: <xsl:apply-templates select="."/></li>
	</xsl:template>
	
	<xsl:template match="added" mode="changes">
		<xsl:text>&#13;</xsl:text>
		<li><b>ДОБАВЛЕНО</b>: <xsl:apply-templates select="."/></li>
	</xsl:template>
	
	<xsl:template match="changed" mode="changes">
		<xsl:text>&#13;</xsl:text>
		<li><b>ИЗМЕНЕНО</b>: <xsl:apply-templates select="."/></li>
	</xsl:template>
	
	<xsl:template match="removed" mode="changes">
		<xsl:text>&#13;</xsl:text>
		<li><b>УДАЛЕНО</b>: <xsl:apply-templates select="."/></li>
	</xsl:template>
